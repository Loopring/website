!(function(c, M) {
  ...
  function o(c) {
    ...
    for (45 === c.charCodeAt(0) && (n = -(M = 1)); M < A.length; M++) {
      p = 60 * p + r(A.charCodeAt(M));
    }
    for (M = 0; M < o.length; M++) (b /= 60), (p += r(o.charCodeAt(M)) * b);
    return p * n;
  }
  function q(c) {
    for (let M = 0; M < c.length; M++) c[M] = o(c[M]);
  }
  function O(c, M) {
  ...
  function L() {
    for (
      var c,
        ...
        A = new t(new Date(a, 0, 1)),
        o = [A],
        b = 1;
      b < 48;
      b++
    ) {
      (M = new t(new Date(a, b, 1))).offset !== A.offset &&
        ((c = (function(c, M) {
          for (var a, A; (A = 6e4 * (((M.at - c.at) / 12e4) | 0)); ) {
            (a = new t(new Date(c.at + A))).offset === c.offset ?
              (c = a) :
              (M = a);
          }
          return c;
        ...
        o.push(c),
        o.push(new t(new Date(c.at + 6e4)))),
      (A = M);
    }
    for (b = 0; b < 4; b++) {
      o.push(new t(new Date(a + b, 0, 1))),
      o.push(new t(new Date(a + b, 6, 1)));
    }
    return o;
  }
  function u(c, M) {
  ...
  function W() {
    ...
    for (
      var a,
        ...
        b = o.length,
        p = (function(c) {
          for (var M, a, A = c.length, o = {}, b = [], p = 0; p < A; p++) {
            for (M in (a = e[c[p].offset] || {})) {
              a.hasOwnProperty(M) && (o[M] = !0);
            }
          }
  ...
  function s(c, M) {
    ...
    return A instanceof f ?
      ...
      ((A = new f(A)), (p[c] = A)) :
      n[c] && M !== s && (a = s(n[c], s)) ?
      ((A = p[c] = new f())._set(a), (A.name = z[c]), A) :
      null;
  }
  function m(c) {
    let M;
  ...
  function C(c) {
    const M = Array.prototype.slice.call(arguments, 0, -1);
    const a = arguments[arguments.length - 1];
    const A = s(a);
    const o = b.utc.apply(null, M);
    return (
      A && !b.isMoment(c) && R(o) && o.add(A.parse(o), 'minutes'), o.tz(a), o
    );
  }
  (a < 2 || (2 == a && A < 6)) &&
    S(
  ...
  (f.prototype = {
    ...
    _index: function(c) {
      for (let M = +c, a = this.untils, A = 0; A < a.length; A++) {
        if (M < a[A]) return A;
      }
    },
    countries: function() {
  ...
  (C.names = function() {
    ...
    const M = [];
    for (c in z) {
      z.hasOwnProperty(c) && (p[c] || p[n[c]]) && z[c] && M.push(z[c]);
    }
    return M.sort();
  }),
  (C.Zone = f),
